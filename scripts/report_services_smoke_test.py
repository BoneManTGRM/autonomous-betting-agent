...
def run_smoke_test() -> None:
    ...
    assert feed["counts"]["total_cards"] == 3
    assert feed["counts"]["official_publish_ready"] == 1
    assert feed["counts"]["client_report_ready"] == 2
    assert feed["counts"]["data_issues"] == 1
    assert feed["groups"]["official_ev"]
    assert feed["groups"]["data_blocked"]
    ...
    assert bundle.feed["counts"]["total_cards"] == 3
