...
PATCH_VERSION = "magazine_display_guard_v2_decimal_rich_context"
GOLD = (241, 184, 45)
...
def _needs_verification(row: Any) -> bool:
    ...
    joined = " ".join(
        ...
        for key in ("odds_source", "data_source", "odds_status", "odds_api_status", "risk", "risk_label", "profit_guard_status")
    )
...
def _trim(text: str, length: int = 86) -> str:
    text = _clean(text)
    if len(text) <= length:
        return text
...
def _prepared_row(row: Any) -> dict[str, Any]:
    ...
    if _needs_verification(data):
        action = _clean(
            ...
            or data.get("agent_decision")
            or data.get("recommendation")
            or data.get("consumer_action")
