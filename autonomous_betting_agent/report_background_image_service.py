...
def _background(background_bytes: bytes | None) -> Image.Image:
    if background_bytes:
        try:
            ...
            src = Image.new("RGB", (PAGE_W, PAGE_H), (30, 38, 55))
    else:
        src = Image.new("RGB", (PAGE_W, PAGE_H), (30, 38, 55))
    ...
    canvas = ImageEnhance.Color(canvas).enhance(0.65)
    canvas = ImageEnhance.Brightness(canvas).enhance(0.58)
    return canvas.filter(ImageFilter.GaussianBlur(radius=0.8)).convert("RGBA")
...
def render_custom_background_card_png(row: Mapping[str, Any], brand: MagazineBrand | Mapping[str, Any] | None = None, *, background_bytes: bytes | None = None, index: int = 0) -> bytes:
    ...
    draw.rectangle((0, 0, PAGE_W, PAGE_H), fill=(0, 0, 0, 78))
    draw.rounded_rectangle((58, 58, PAGE_W - 58, PAGE_H - 58), radius=34, outline=BORDER, width=3, fill=(0, 0, 0, 45))
    draw.text((92, 92), brand_name.upper(), font=_bold(28), fill=GOLD)
    draw.text((92, 138), sport.upper(), font=_font(24), fill=MUTED)
    y = 225
    for line in wrap(title, width=22)[:3]:
        y = _center(draw, y, line, _bold(58), fill=INK) + 8
    y += 18
    draw.rounded_rectangle((170, y, PAGE_W - 170, y + 70), radius=28, fill=PANEL_STRONG, outline=BORDER, width=2)
    _center(draw, y + 15, action, _bold(31), fill=GOLD)
    y += 125
    for bullet in _bullets(row):
        draw.text((120, y), "•", font=_bold(42), fill=GOLD)
        y = _wrapped(draw, 170, y, bullet, _font(38), width=34, fill=INK, max_lines=3, line_gap=11)
        y += 34
        if y > 940:
            break
    draw.rounded_rectangle((250, 1030, PAGE_W - 250, 1165), radius=28, fill=PANEL_STRONG, outline=BORDER, width=2)
    _center(draw, 1054, "TENDENCY", _bold(35), fill=GOLD)
    _center(draw, 1110, _trend(row)[:28], _bold(38), fill=INK)
    return _png(image)


def render_custom_background_summary_png(cards: pd.DataFrame, brand: MagazineBrand | Mapping[str, Any] | None = None, *, background_bytes: bytes | None = None, top_n: int = 8) -> bytes:
    ...
    draw.rectangle((0, 0, PAGE_W, PAGE_H), fill=(0, 0, 0, 88))
    _center(draw, 86, brand_name.upper(), _bold(32), fill=GOLD)
    _center(draw, 148, title, _bold(56), fill=INK)
    y = 270
    draw.text((92, y), "TOP CARDS", font=_bold(38), fill=GOLD)
    y += 72
    for idx, (_, row) in enumerate(frame.iterrows(), start=1):
        ...
        draw.rounded_rectangle((84, y, PAGE_W - 84, y + 102), radius=24, fill=PANEL, outline=BORDER, width=2)
        draw.text((116, y + 16), f"{idx}. {event[:42]}", font=_bold(27), fill=INK)
        draw.text((116, y + 58), pick[:42], font=_font(24), fill=MUTED)
        draw.text((PAGE_W - 392, y + 38), action[:26], font=_bold(24), fill=GOLD)
        y += 124
        if y > 1180:
            break
    return _png(image)
