...
MARKET_LABELS = {'spreads': 'Point spread', 'totals': 'Game total'}
...
def validate_api_key(api_key: str) -> str:
    ...
    if looks_like_placeholder_key(key):
        raise RuntimeError('Invalid or placeholder Odds API key. Replace it with a real key before scanning live odds.')
    return key
...
def _get_json(path: str, params: Dict[str, Any]) -> Any:
    ...
    try:
        ...
        if response.status_code == 401:
            raise RuntimeError('Odds API rejected the key with 401 Unauthorized.') from exc
        if response.status_code == 429:
            raise RuntimeError('Odds API quota/rate limit reached.') from exc
        raise
...
def fetch_odds(api_key: str, sport_key: str, regions: str = 'us,eu,uk', markets: str = 'h2h,spreads,totals', odds_format: str = 'decimal') -> List[Dict[str, Any]]:
    if not is_three_api_supported_sport(sport_key):
        return []
    return list(_get_json(f'/v4/sports/{sport_key}/odds/', {
        'apiKey': api_key,
        'regions': regions,
        'markets': markets,
...
def _market_lines(bookmakers: Iterable[Dict[str, Any]], market_key: str) -> List[MarketLine]:
    ...
    for (name, point), entries in line_map.items():
        ...
        raw_probability = 1.0 / avg_price if avg_price > 1.0 else 0.0
        raw_rows.append(MarketLine(
            market=market_key,
            name=name,
            point=point,
    ...
    # Normalize within comparable line groups. Spreads compare both sides of the same point magnitude.
    groups: Dict[Tuple[str, Optional[float]], float] = {}
    for row in raw_rows:
        point_key = None if row.point is None else abs(float(row.point))
        groups[(row.market, point_key)] = groups.get((row.market, point_key), 0.0) + float(row.raw_probability or 0.0)
    normalized: List[MarketLine] = []
    for row in raw_rows:
        point_key = None if row.point is None else abs(float(row.point))
        total = groups.get((row.market, point_key), 0.0)
        probability = float(row.raw_probability or 0.0)
        normalized_probability = probability / total if total > 0 else probability
        normalized.append(MarketLine(
            market=row.market,
            name=row.name,
            point=row.point,
    ...
    normalized.sort(key=lambda item: (item.market, item.point if item.point is not None else 0.0, item.name))
    return normalized
...
def summarize_event(event: Dict[str, Any]) -> Optional[LiveEventSummary]:
    ...
    price_map = _prices_by_outcome(bookmakers)
    spreads = _market_lines(bookmakers, 'spreads')
    totals = _market_lines(bookmakers, 'totals')
    ...
    # Keep spread/total-only events. This fixes the old h2h-only bottleneck.
    if not outcomes and not spreads and not totals:
        return None
    favorite = outcomes[0].name if outcomes else ''
    favorite_probability = outcomes[0].normalized_probability if outcomes else 0.0
    cycle_notes = [
        'TEST: pulled live h2h, spread, and total market prices when available.',
        'DETECT: keeps spread/total-only events instead of dropping them for missing h2h.',
        'REPAIR: averaged prices across books, found best prices, and normalized comparable prices.',
        'VERIFY: Pro Predictor can now create h2h, spread, and total rows separately.',
    ]
...
def _apply_calibration(summary: LiveEventSummary, calibrator: ProbabilityCalibrator) -> LiveEventSummary:
    ...
    for outcome, adjusted_probability in zip(summary.outcomes, adjusted):
        calibrated_outcomes.append(OutcomePrice(
            name=outcome.name,
...
def scan_market(api_key: str, sport_key: str, regions: str = 'us,eu,uk', max_events: int = 25, markets: str = 'h2h,spreads,totals', learned_state_path: str | Path | None = 'learned_state.json') -> List[LiveEventSummary]:
    if not is_three_api_supported_sport(sport_key):
        return []
