...
if result.empty:
    ...
    if not skipped.empty:
        st.subheader(t('skipped')); st.dataframe(skipped, use_container_width=True, hide_index=True)
    st.stop()
strength = scanner_strength_summary(result); health = page_health(result, page='scanner_pro')
st.success(t('stored'))
cols = st.columns(8)
cols[0].metric(t('rows'), len(result)); cols[1].metric(t('sports'), result['sport_key'].nunique() if 'sport_key' in result else 0); cols[2].metric(t('events'), result['event_id'].nunique() if 'event_id' in result else 0); cols[3].metric(t('books'), round(float(pd.to_numeric(result.get('bookmaker_count', pd.Series(dtype=float)), errors='coerce').fillna(0).mean()), 2)); cols[4].metric(t('best_price_rows'), int(result.get('best_price', pd.Series(dtype=str)).fillna('').astype(str).str.strip().ne('').sum()) if 'best_price' in result else 0); cols[5].metric(t('avg_strength'), 'N/A' if strength['avg_score'] is None else strength['avg_score']); cols[6].metric(t('premium'), strength['premium_scan']); cols[7].metric('Next', health['next_action'])
st.subheader(t('handoff')); st.dataframe(page_health_frame(result, page='scanner_pro'), use_container_width=True, hide_index=True)
...
with tabs[0]: st.dataframe(result[result['market_type'].eq('h2h')] if 'market_type' in result else result, use_container_width=True, hide_index=True)
with tabs[1]: st.dataframe(result[result['market_type'].eq('spreads')] if 'market_type' in result else pd.DataFrame(), use_container_width=True, hide_index=True)
with tabs[2]: st.dataframe(result[result['market_type'].eq('totals')] if 'market_type' in result else pd.DataFrame(), use_container_width=True, hide_index=True)
with tabs[3]: st.dataframe(result, use_container_width=True, hide_index=True)
