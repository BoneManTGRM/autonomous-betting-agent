...
PAGE_CONTRACT_FIELDS = ("schema_version", "workspace_id", "cards_id", "cards_hash", "mode", "cards_status", "card_count", "bet_count", "watch_count", "avoid_count", "recommendation_cards", "marco_cards", "completion_checks", "safety_gates", "preview_only", "files_written", "live_changes")

TEXT = {
    "en": {
        "title": "Recommendation Cards", "caption": "Create clean report cards from the Market Optimizer result.", "help": "Run Market Optimizer first. This page will use that result automatically. Manual paste boxes are under Advanced.",
        "workspace_id": "Workspace ID", "source": "Source check", "run": "Build cards", "advanced": "Advanced manual input", "optimizer_json": "Manual Market Optimizer JSON", "market_csv": "Optional Market Hunter rows CSV", "chain_csv": "Optional Chain Builder rows CSV", "context_csv": "Optional sports context CSV",
        "summary": "Summary", "cards": "Cards", "marco": "Pro view", "checks": "Checks", "safety": "Safety details", "download_json": "Download cards JSON", "download_csv": "Download cards CSV", "download_marco": "Download pro view JSON", "download_checks": "Download checks CSV", "download_manifest": "Download manifest JSON", "preview_only": "Preview only", "no_files": "No files written.", "no_live": "No live changes.", "no_report": "Build cards to view outputs.", "no_source": "No Market Optimizer result found. Run Market Optimizer first or paste JSON in Advanced.",
    },
    "es": {
        "title": "Tarjetas de Recomendación", "caption": "Crea tarjetas limpias desde Market Optimizer.", "help": "Ejecuta Market Optimizer primero. Esta página usa ese resultado automáticamente. Los campos manuales están en Avanzado.",
        "workspace_id": "ID de workspace", "source": "Revisión de fuente", "run": "Construir tarjetas", "advanced": "Entrada manual avanzada", "optimizer_json": "JSON manual de Market Optimizer", "market_csv": "CSV opcional Market Hunter", "chain_csv": "CSV opcional Chain Builder", "context_csv": "CSV opcional contexto deportivo",
        "summary": "Resumen", "cards": "Tarjetas", "marco": "Vista Pro", "checks": "Checks", "safety": "Detalles de seguridad", "download_json": "Descargar JSON tarjetas", "download_csv": "Descargar CSV tarjetas", "download_marco": "Descargar JSON pro", "download_checks": "Descargar CSV checks", "download_manifest": "Descargar JSON manifest", "preview_only": "Solo preview", "no_files": "No escribe archivos.", "no_live": "No hace cambios live.", "no_report": "Construye tarjetas para ver outputs.", "no_source": "No hay resultado de Market Optimizer. Ejecútalo primero o pega JSON en Avanzado.",
    },
}
...
metrics = st.columns(7); metrics[0].metric("status", report.get("cards_status", "")); metrics[1].metric("cards", report.get("card_count", 0)); metrics[2].metric("bet", report.get("bet_count", 0)); metrics[3].metric("watch", report.get("watch_count", 0)); metrics[4].metric("avoid", report.get("avoid_count", 0)); metrics[5].metric("warn", report.get("warn_count", 0)); metrics[6].metric("fail", report.get("fail_count", 0))
tabs = st.tabs([t("summary"), t("cards"), t("marco"), t("checks")])
with tabs[0]: st.json({field: report.get(field) for field in PAGE_CONTRACT_FIELDS})
with tabs[1]: st.dataframe(pd.DataFrame(report.get("recommendation_cards") or []), use_container_width=True, hide_index=True)
with tabs[2]: st.json(report.get("marco_cards") or [])
with tabs[3]: st.dataframe(pd.DataFrame(report.get("completion_checks") or []), use_container_width=True, hide_index=True)
...
st.download_button(t("download_marco"), export_marco_cards_json(report).encode("utf-8"), file_name=f"aba_pro_cards_{suffix}.json", mime="application/json", key=f"pro_cards_marco_{safe_text(report.get('cards_hash'))}")
