...
TEXT = {
    "en": {
        ...
        "upload": "Optional graded CSV for this Reparodynamics Shadow Mode scan",
        ...
        "phase3b_explanation": "Phase 3B does not change picks directly. It scans graded results, detects drift from real rows, finds duplicate-event issues, generates repair candidates, and evaluates those candidates in Shadow Mode only. No live model, pick, confidence, bankroll, sportsbook, or bet-tier changes are allowed.",
        "final": "Final rule",
        "warning": "This page can run a real Shadow Mode scan and write an audit event. Shadow Mode is ON for counterfactual evaluation, but live repairs, TGRM, RYE scoring activation, confidence changes, bet-tier changes, bankroll changes, sportsbook changes, filters, and model mutation remain forbidden.",
    },
    "es": {
        "title": "Reparodynamics",
        ...
        "upload": "CSV calificado opcional para este escaneo Shadow Mode de Reparodynamics",
        "upload_loaded": "Filas subidas cargadas para escaneo Shadow Mode.",
        "run_now": "Ejecutar escaneo Shadow Mode Fase 3B ahora",
        "run_success": "Escaneo Shadow Mode de Reparodynamics completado y evento de auditoría escrito.",
        "scan_summary": "Resumen del último escaneo",
        "no_run": "Todavía no hay ejecución registrada.",
        "phase3b_explanation": "La Fase 3B no cambia picks directamente. Escanea resultados calificados, detecta deriva solo con filas reales, encuentra problemas de eventos duplicados, genera candidatos de reparación y evalúa esos candidatos solo en Shadow Mode. No se permiten cambios en vivo al modelo, picks, confianza, bankroll, sportsbook ni niveles de apuesta.",
        "final": "Regla final",
        "warning": "Esta página puede ejecutar un escaneo real en Shadow Mode y escribir un evento de auditoría. Shadow Mode está ON para evaluación contrafactual, pero las reparaciones en vivo, TGRM, activación de puntuación RYE, cambios de confianza, niveles de apuesta, bankroll, sportsbook, filtros y mutación del modelo siguen prohibidos.",
    },
}
...
ES_LIST_MAP = {
    ...
    "Evaluate pattern candidates in Shadow Mode before promotion.": "Evaluar candidatos de patrón en Shadow Mode antes de promoverlos.",
    ...
    "Phase 3B enables Shadow Mode evaluation only.": "La Fase 3B permite solo evaluación en Shadow Mode.",
...
if LANG == "es":
    st.write("Reparodynamics es la doctrina operativa de autorreparación medida. ABA observa primero, diagnostica con cuidado, preserva la integridad de los datos, conserva energía de reparación y prueba reparaciones en Shadow Mode antes de considerar cualquier cambio dirigido para aprobación manual.")
else:
